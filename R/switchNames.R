switchNames <- function(DT, simple = TRUE) {
	sNames <- c(
		"rn", "Sensor", "Source", 
        "SensorHeight", "SourceArea", 
        "CE", "CE_se", "CE_lo", "CE_hi", 
        "uCE", "uCE_se", "uCE_lo", "uCE_hi", 
        "vCE", "vCE_se", "vCE_lo", "vCE_hi", 
        "wCE", "wCE_se", "wCE_lo", "wCE_hi", 
        "N_TD", "TD_Time_avg", "TD_Time_max", 
        "Max_Dist", "UCE", "Subset_seed", 
        "z_sWu", "Ustar", "L", "Zo", 
        "sUu", "sVu", "sWu", 
        "WD", "d", "kv", "A", "MaxFetch", "N0", 
        "alpha", "bw", "C0", "Sensor_Swustar"
		) 
	cNames <- c(
		...
		"Maximum Distance [m]", "uC/E", "Catalog Subset Seed", 
        "SigmaW/Ustar Height [m]", "Ustar [m/s]", "L [m]", "Zo [m]", 
        "SigmaU/Ustar [-]", "SigmaV/Ustar [-]", "SigmaW/Ustar [-]", 
        "WD [deg N]", "d [m]", "kv", "A", "MaxFetch", "N0", 
        "alpha", "bw", "C0", "SigmaW/Ustar @ Sensor Height"
		)
	if (simple) {
		from <- cNames
		to <- sNames
